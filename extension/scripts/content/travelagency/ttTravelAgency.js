...
(async () => {
	...
	if (mobile || tablet) {
		...
		document.find(".torn-tabs").addEventListener("click", (event) => {
			...
			if (parent) {
				triggerCustomListener(EVENT_CHANNELS.TRAVEL_SELECT_TYPE, { type: parent.find(".travel-name").textContent.toLowerCase() });
			}
		});
	} else {
		for (const type of document.findAll(".travel-agency .torn-tabs > li:not([data-state='disabled'])")) {
			type.addEventListener("click", async () => {
				triggerCustomListener(EVENT_CHANNELS.TRAVEL_SELECT_TYPE, { type: type.find(".travel-name").textContent.toLowerCase() });
			});
		}
